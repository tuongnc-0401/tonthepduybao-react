...
import {
  DEBT_FULL_TABLE_COLUMNS,
  DEBT_SCREW_TABLE_COLUMNS,
} from "~/modules/table";
...
export default function EditDebt() {
  ...
  const selectProperty = (propId) => {
    const property = propertyStore.allProperty.find(
      (item) => item.id === propId,
    );
  ...
  const deselectProperty = (propId) => {
    ...
    if (!isEmptyItems && newSelectedProperties.length === 0) {
      ...
      Modal.confirm({
        ...
        content:
          "Khi xoá thuộc tính, các thuộc tính của sản phẩm cũng sẽ bị xoá theo, bạn có chắc muốn xoá không?",
        okText: "Có",
        okType: "primary",
        cancelText: "Không",
        onOk() {
          setSelectedProperties((prev) =>
            prev.filter((item) => item.id !== propId),
          );
          setFormState((s) => ({
            ...s,
        ...
        onCancel() {
          setFormState((s) => ({
            ...s,
            propertyIds: [...s.propertyIds, propId],
          }));
        },
      });
    } else {
  ...
  const addDebtItem = () => {
    if (selectedProperties.length === 0) {
      ...
      setFormState((s) => ({
        ...s,
        items: [...s.items, cloneDeep(emptyDebtItem)],
      }));
  ...
  const validateItems = (items) => {
    if (!items || items.length === 0)
      return { tableItems: "Vui lòng thêm sản phẩm cho công nợ" };
  ...
  const validate = () => {
    ...
    if (!propertyIds || propertyIds.length === 0)
      errors.propertyIds = "Thuộc tính là trường băt buộc";
  ...
  const getRowClassName = (_, index) => {
    return formErrors.items && formErrors.items.includes(index)
            ...
            {selectedProperties.map((prop, propIndex) => (
              <Select
                ...
                onChange={(val) =>
                  updateItem(index, (item) => ({
                    ...item,
                    properties: { ...item.properties, [prop.id]: val },
                  }))
                }
              />
            ))}
      ...
      return {
        ...
        render: (_, record, index) => (
          <Input
            ...
            value={record.weight}
            onChange={(e) =>
              changeAvgProportion(index, e.target.value, record.quantity)
            }
          />
        ),
      };
      ...
      return {
        ...
        render: (_, record, index) => (
          <Input
            ...
            onChange={(e) =>
              changeAvgProportion(index, record.weight, e.target.value)
            }
          />
        ),
      };
        ...
        render: (_, record, index) => (
          <div className="flex items-center">
            <Button
              type="link"
              danger
              onClick={() => deleteDebtItem(index, record)}
            >
  ...
  return (
    <section>
      <Heading
        title={`${MENU.EDIT_DEBT.name}: [${debt.id}]`}
        className="items-start"
      >
        <div className="flex items-center mb-4">
          <Button
            className="w-[120px] mr-0 flex items-center justify-center"
            onClick={() => navigate(-1)}
          >
            Huỷ bỏ
          </Button>
          <Button
            type="primary"
            className="w-[120px] ml-4 mr-0 flex items-center justify-center"
            onClick={submit}
          >
      ...
      <p className="text-right italic mb-0 mt-2 text-xl text-red-500">
      ...
      <div className="grid grid-cols-12 gap-x-8 mt-2">
        <div className="col-span-9">
          <label>
            <span className="text-red-500">*</span> Mã công nợ
          </label>
          <Input value={formState.id} placeholder="Nhập mã công nợ" disabled />
          {formErrors.id && (
            <p className="mb-0 text-red-500 mt-0.5 text-[12px]">
              {formErrors.id}
            </p>
          )}
        </div>
      ...
      <div className="grid grid-cols-12 gap-x-8 mt-2">
        <div className="col-span-3 mb-4">
          <label>
            <span className="text-red-500">*</span> Ngày nhập công nợ
          </label>
          <DatePicker
            ...
            value={
              formState.date
                ? dayjs(formState.date, moment.MOMENT_FORMAT.YYYYMMDD)
                : null
            }
            className="w-full mt-1"
            onChange={(_, dateStr) => {
              setFormState((s) => ({ ...s, date: dateStr }));
              clearValidate("date");
            }}
          />
          {formErrors.date && (
        ...
        <div className="col-span-3 mb-4">
          ...
          <Select
            ...
            className="w-full mt-1"
            onChange={(val) => {
              setFormState((s) => ({ ...s, customerId: val }));
        ...
        <div className="col-span-3 mb-4">
          ...
          <Select
            ...
            onChange={(val) => {
              setFormState((s) => ({ ...s, propertyIds: val }));
              clearValidate("propertyIds");
            }}
          />
        ...
        <div className="col-span-3 flex justify-end pt-[24px]">
          ...
          <Button
            type="primary"
            ghost
            className="min-w-[120px] flex items-center ml-4"
            onClick={addDebtItem}
          >
      ...
      <div className="flex justify-between my-8">
        <div>
          ...
          <span className="font-normal ml-2">
            {formState.items.length} sản phẩm
          </span>
