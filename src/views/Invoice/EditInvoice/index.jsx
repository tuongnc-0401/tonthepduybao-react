...
const emptyProductItem = { id: null, product: null, quantity: 0, unitPrice: 0 }
...
export default function EditInvoice() {
  ...
  const calPrice = (items) => {
    const tp = (items || formState.items).reduce(
      (a, item) => a + (Number(item.unitPrice) || 0) * (Number(item.quantity) || 0),
      0
    )
    setTotalPrice(tp)
  }
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        await productStore.getAllOption({ branchId: inv.branch.id })

        commonStore.setBreadcrumbs([
          MENU.INVOICE,
          { name: MENU.EDIT_INVOICE.name, path: MENU.EDIT_INVOICE.path + inv.id }
        ])

        const items = (inv.invoiceProducts || []).map((item) => ({
          id: item.id,
        ...
        setFormState({
          ...
          shippingAddressId: inv.shippingAddress.id,
          note: inv.note,
          items
        })

        calPrice(items)
        await initShippingAddress(inv.customer.id)
      } catch {
  ...
  const validate = () => {
    const { date, branchId, customerId, shippingAddressId, items } = formState
    const errors = {}
  ...
  const submit = async () => {
    if (!validate()) return
    const { date, branchId, customerId, note } = formState
    ...
    try {
      await invoiceStore.update({ id, date, branchId, customerId, note, items })
  ...
  const changeCustomer = async (customerId) => {
    ...
    await initShippingAddress(customerId)
  }
  ...
  const deleteShippingAddress = async (saId) => {
    try {
      await shippingAddressStore.delete(saId)
      await initShippingAddress(formState.customerId)
  ...
  const updateDefaultShippingAddress = async (saId) => {
    if (!formState.customerId) { mc.error('Vui lòng chọn khách hàng để thêm địa chỉ giao hàng!'); return }
    try {
      await shippingAddressStore.updateDefault({ id: saId, customerId: formState.customerId })
      await initShippingAddress(formState.customerId)
      mc.success(MSG.UPDATE_SUCCESS)
    } catch {
      mc.error(MSG.UPDATE_FAILED)
  ...
  return (
    <section>
      <Heading title={MENU.EDIT_INVOICE.name} className="items-start">
      ...
      <div className="w-9/12 mx-auto">
        <div className="grid grid-cols-12 gap-x-8 mt-2 mb-4">
          <div className="col-span-4">
            ...
            <Input value={formState.id} placeholder="Nhập số hoá đơn" maxLength={100} disabled />
          </div>
          <div className="col-span-4">
