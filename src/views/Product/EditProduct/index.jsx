...
import { Button, Divider, Select, Input, DatePicker } from 'antd'
...
export default function EditProduct() {
  ...
  const clearValidate = (errorName) => {
    setFormErrors((prev) => { const n = { ...prev }; delete n[errorName]; return n })
  }

  const changeType = async (type, propDetailIds = []) => {
    await propertyStore.getAllByType({ type })
    ...
    sortedProperties.forEach((prop) => {
      ...
      if (propDetailIds.length !== 0) {
        const found = prop.items.find((pi) => propDetailIds.includes(pi.id))
  ...
  useEffect(() => {
    ...
    const loadProduct = async () => {
      try {
        ...
        await changeType(product.type, product.properties.map((p) => p.id))
      } catch (e) {
        console.log(e)
      }
    }
    ...
    <section>
      ...
      <div className="grid grid-cols-12 gap-x-8 mt-2">
        ...
        <div className="col-span-3">
          ...
          <Select value={formState.parent} allowClear options={[]} placeholder="Chọn sản phẩm gốc" className="w-full" onChange={(val) => setFormState((s) => ({ ...s, parent: val }))} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-x-8 mt-8">
        ...
        <div className="col-span-3">
          ...
          <Input type="number" min={0} value={formState.quantity} onChange={(e) => { const q = e.target.value; setFormState((s) => ({ ...s, quantity: q })); calSizeCalculator(formState.size, q, formState.type) }} />
      ...
      {formState.type === TYPE_KEY.CORRUGATED && (
        <div className="grid grid-cols-12 gap-x-8 mt-8">
          <div className="col-span-3">
            ...
            <Input type="number" min={0} value={formState.size} onChange={(e) => { const sz = e.target.value; setFormState((s) => ({ ...s, size: sz })); calSizeCalculator(sz, formState.quantity, formState.type) }} />
          </div>
          <div className="col-span-3">
          ...
          {selectedProperties.map((prop) => (
            <div key={prop.id} className="col-span-3 flex items-center">
              ...
              <Select
                ...
                className="w-full"
                onChange={(val) => { setFormState((s) => ({ ...s, properties: { ...s.properties, [prop.id]: val } })); clearValidate('properties') }}
              />
            </div>
          ))}
        </div>
        {formErrors.properties && <p className="mb-0 text-red-500 mt-0.5 text-[12px]">{formErrors.properties}</p>}
      </div>
    </section>
  )
}
